...
def predict(
    ...
    city_metrics = {}
    if "city" in emp_df.columns:
        grouped_city = emp_df.groupby("city")
        for city, city_df in grouped_city:
            city_metrics[str(city)] = {
                "attrition_rate": round(float(city_df["leaveornot"].mean()) * 100, 1)
                if "leaveornot" in city_df.columns else 0.0,
                "employees": int(len(city_df)),
                "avg_tenure": round(float(city_df["experienceincurrentdomain"].mean()), 1)
                if "experienceincurrentdomain" in city_df.columns else 0.0,
            }
